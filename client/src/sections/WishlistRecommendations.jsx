...
const WishlistRecommendations = ({ wishlist }) => {
  ...
  const { data: recommendations = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.data.products.filter((p) => !excludeSet.has(String(p._id || p.id)));
    },
    staleTime: 300000,
  });
    ...
    <section className="w-full bg-white pt-14 pb-4" aria-label="More products for you">
      <div className="max-w-[1400px] mx-auto">
        ...
        {recommendations.length > 0 && (
          <div>
            <h3 className="text-[20px] font-bold text-[#0F1111] mb-6 border-b border-[#EAEAEC] pb-3">
              Recommended for You
            </h3>
            <p className="text-[#6B7280] -mt-4 mb-6 text-[14px]">
