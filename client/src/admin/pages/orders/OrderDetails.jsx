    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 3, flexWrap: "wrap" }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography variant="h5" sx={{ fontWeight: 800, color: "var(--color-admin-text)", lineHeight: 1.2, overflowWrap: "break-word", fontSize: { xs: "1.125rem", sm: "1.375rem", md: "1.5rem" } }}>{order.orderNumber}</Typography>
          <Typography variant="body2" sx={{ color: "var(--color-admin-muted)", fontWeight: 500, overflowWrap: "break-word" }}>{order.customer?.email || "—"}</Typography>
        </Box>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
        <Box sx={{ p: 3, border: "1px solid var(--color-admin-border)", borderRadius: "var(--radius-admin-card)" }}>
          ...
          {order.items?.length > 0 && (
            <Box sx={{ mt: 3, overflowX: "auto" }}>
              ...
              <Table size="small" sx={{ minWidth: 520, "& th, & td": { fontSize: "0.8125rem", color: "var(--color-admin-text)", borderColor: "var(--color-admin-border)" } }}>
                <TableHead>
                  <TableRow>
        ...
        <Box sx={{ p: 3, border: "1px solid var(--color-admin-border)", borderRadius: "var(--radius-admin-card)" }}>
          ...
          <Typography variant="body2" sx={{ color: "var(--color-admin-text-secondary)", wordBreak: "break-word" }}>{order.shippingAddress}</Typography>
        </Box>
